...
public class JwtTokenFilter extends GenericFilterBean {
    ...
    // this is called for every request that comes in
    // (unless its filtered out before in the chain)
    @Override
    public void doFilter(ServletRequest req, ServletResponse res, FilterChain filterChain) throws IOException, ServletException {
        ...
        if (token != null && jwtTokenServices.validateToken(token)) {
            ...
            // If this code does not run, the request will fail
            // for routes that are configured to need authentication
